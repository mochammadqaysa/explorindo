  ...
  <table id="table">
      ...
      <tbody>
        ...
        <tr>
            ...
            <td style="text-align: right">{!! !is_null($item['masuk']['jumlah']) && $item['masuk']['jumlah'] != '0.00' ? '<button class="direct" onClick="navigateToMasuk(\''.$date1.'\',\''.$date2.'\',\''. $item['barang_id'] .'\')" >' . $item['masuk']['jumlah'] ."</button>" : "" !!}</td>
            <td style="text-align: right">{!! !is_null($item['masuk']['netto']) && $item['masuk']['netto'] != '0.00' ? '<button class="direct" onClick="navigateToMasuk(\''.$date1.'\',\''.$date2.'\',\''. $item['barang_id'] .'\')" >' . Utils::decimal($item['masuk']['netto'],3) ."</button>" : "" !!}</td>
            <td style="text-align: right">{!! !is_null($item['keluar']['jumlah']) && $item['keluar']['jumlah'] != '0.00' ? '<button class="direct" onClick="navigateToKeluar(\''.$date1.'\',\''.$date2.'\',\''. $item['barang_id'] .'\')" >' . $item['keluar']['jumlah'] ."</button>" : "" !!}</td>
            <td style="text-align: right">{!! !is_null($item['keluar']['netto']) && $item['keluar']['netto'] != '0.00' ? '<button class="direct" onClick="navigateToKeluar(\''.$date1.'\',\''.$date2.'\',\''. $item['barang_id'] .'\')" >' . Utils::decimal($item['keluar']['netto'],3) ."</button>" : "" !!}</td>
...
<script>
  function navigateToMasuk(from, to, barang) {
      // Use window.location.href to navigate to a new page
      window.location.href = `{{ route('result-report.barang-masuk') }}?periode=${from}+-+${to}&barang=${barang}`;
  }
  function navigateToKeluar(from, to, barang) {
      // Use window.location.href to navigate to a new page
      window.location.href = `{{ route('result-report.barang-keluar') }}?periode=${from}+-+${to}&tipe=semua&barang=${barang}`;
  }
