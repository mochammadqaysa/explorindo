    ...
    <table class="table display nowrap" style="width:100%" id="table-bahanmasuk-item">
        ...
        <tbody>
            ...
            <tr>
                ...
                @endif
                @if (@$bahanMasuk->tipe == "impor")
                    <td>IDR {!! Utils::decimal($item->nilai * $bahanMasuk->kurs ?? 0, 2) !!}</td>
                @else 
...
<script>
    $(() => {
        let tablebahan = $("#table-bahanmasuk-item").DataTable({
          responsive: {
              details: {
              type: 'column'
              }
        ...
        tablebahan.rows(':not(.parent)').nodes().to$().find('td:first-child').trigger('click');
    })
</script>