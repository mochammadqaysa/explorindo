...
<script>
  ...
  function validateStep1() {
    let kosong = ''
    let validateTransaksi = $('[name="transaksi"]:checked').val()
  ...
  function validateStep2() {
    ...
    $('#dynamic-form .form-item').each(function (index) {
      ...
      if (!bahanValue) {  // If the field is empty
        ...
        let transaksi = $('[name="transaksi"]:checked').val()
        if (transaksi == 'keluar') {
  ...
  $(() => {
    ...
    $('input[name="transaksi"]').on('change', function() {
        ...
        if (selectedTransaksi == "keluar") {
          // set produksi
          // produksi
          $('[name="bagian"]').val("612e0012-e0d1-46cb-8bb2-e0c8e36cc19f").trigger('change');
        } else {
          // gudang 
          $('[name="bagian"]').val("ca0ac694-1456-4911-9a1e-6b8e0991e02c").trigger('change');
